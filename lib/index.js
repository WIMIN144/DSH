...
var MENU_TIPS = {
  size: '挂件整体缩放（0.6 - 2.5 倍）',
  bubble: '开启/关闭思考气泡（余额、随机台词、消耗提示都显示在这里）',
  ...
  persist: '气泡常驻显示内容：峰谷时段 = 常驻显示当前高峰/空闲时段；余额 = 常驻显示余额+今日已用；对话气泡 = 随机台词自动轮播；选「无」= 普通模式（5 秒自动收起，点击气泡出随机台词）',
  peak: '峰谷时段显示样式（气泡里高峰/空闲时段的叫法）：默认 / 梁文峰谷 / !?强强?!',
  lock: '梁文峰时段锁定：高峰时段（工作日 9:00-12:00 / 14:00-18:00）隐藏 DeepSeek 会话的发送按钮并拦截键盘发送，空闲时段自动恢复',
  car: '高峰锁定气泡两个视图（锁定提示 ⇄ 峰时倒计时）的轮播间隔秒数；填 0 表示不轮播',
...
peakSelect.className = 'dshwv-sound dshwv-sound-sm'
peakSelect.title = MENU_TIPS.peak
...
bubbleToggle.title = MENU_TIPS.bubble
...
persistSelect.className = 'dshwv-sound dshwv-sound-sm'
persistSelect.title = MENU_TIPS.persist
function persistOpt(v, label) {
  var o = document.createElement('option')
  o.value = v
  o.textContent = label
  return o
}
persistSelect.appendChild(persistOpt('', '无'))
persistSelect.appendChild(persistOpt('peak', '峰谷时段'))
persistSelect.appendChild(persistOpt('balance', '余额'))
persistSelect.appendChild(persistOpt('chat', '对话气泡'))
persistSelect.addEventListener('change', function () { setPersistMode(persistSelect.value) })
...
var row1 = menuRow()
row1.appendChild(menuLabel('大小', true, MENU_TIPS.size))
row1.appendChild(scaleInput)
row1.appendChild(scaleNumber)
row1.appendChild(menuLabel('气泡开关', null, MENU_TIPS.bubble))
row1.appendChild(bubbleToggle)
var row2 = menuRow()
...
var row3 = menuRow()
row3.appendChild(menuLabel('用量', true, MENU_TIPS.usage))
row3.appendChild(usageSelect)
row3.appendChild(menuLabel('常驻显示', null, MENU_TIPS.persist))
row3.appendChild(persistSelect)
var row4 = menuRow()
row4.appendChild(menuLabel('峰谷', true, MENU_TIPS.peak))
row4.appendChild(peakSelect)
row4.appendChild(menuLabel('高峰锁定', null, MENU_TIPS.lock))
row4.appendChild(peakLockToggle)
row4.appendChild(menuLabel('轮播', null, MENU_TIPS.car))
row4.appendChild(lockCarInput)
row4.appendChild(menuLabel('秒'))
var menuSep0 = document.createElement('div')
menuSep0.className = 'dshwv-menu-sep'
var menuSep1 = document.createElement('div')
menuSep1.className = 'dshwv-menu-sep'
var row7 = menuRow()
row7.appendChild(menuLabel('每轮消耗提示', true, MENU_TIPS.turn))
row7.appendChild(turnCostToggle)
row7.appendChild(menuLabel('自动关闭', null, MENU_TIPS.close))
row7.appendChild(turnCostCloseInput)
row7.appendChild(menuLabel('秒'))
var row9 = menuRow()
row9.appendChild(menuLabel('避让滚动条', true, MENU_TIPS.gap))
row9.appendChild(scrollGapToggle)
row9.appendChild(menuLabel('宽度', null, MENU_TIPS.gapw))
...
bubbleBox.addEventListener('click', function (e) {
  ...
  if (bubbleRandomActive) {
    // 再次点击：无常驻时关闭；常驻/锁定时回到基础视图
    if (lockActive) showLockBubble()
...
function showBubble() {
  ...
  if (lockActive) { showLockBubble(); return }
...
function hideCostBubble() {
  ...
  if (bubbleOn && (persistMode() !== null || lockActive)) showBubble()
  else hideBubble()
}
...
function render() {
  ...
  if (bubbleShown && !bubbleRandomActive && lockActive) { applyLockView(); return }
...
// —— 梁文峰时段锁定 ——
// 勾选后处于高峰时段（工作日 9-12 / 14-18，北京时间；周末全天谷价不算）
// 且当前模型是 DeepSeek 时：隐藏发送按钮 + 拦截键盘发送，气泡弹锁定提示。
function lockLines() {
  return [
    { t: '梁文峰时段', s: 'A', c: '' },
    { t: '锁定中', s: 'P', c: '#e0433f' },
...
// 轮播第二视图：峰时倒计时（HH:MM:SS 红字）
function lockCountdownLines() {
  return [
    { t: '距离梁文谷还有：', s: 'A', c: '' },
    { t: peakRemainText() || '--:--:--', s: 'P', c: '#e0433f' },
    { t: '亿万鲸子仍需忍耐', s: 'C', c: '' },
  ]
}
// 锁定气泡双视图轮播：lock 提示 ⇄ countdown 倒计时，间隔 lockCarouselMs（0 = 不轮播）
...
function applyLockView() {
  applyBubbleLines(lockView === 'countdown' ? lockCountdownLines() : lockLines())
}
// 倒计时视图期间每秒校时（HH:MM 分钟位变化时刷新；峰时结束立即切回锁定视图）
function startCountdownTick() {
  if (lockTickTimer) { clearInterval(lockTickTimer); lockTickTimer = null }
  if (lockView !== 'countdown' || !lockActive || !bubbleShown) return
  lockTickTimer = setInterval(function () {
    if (lockView !== 'countdown' || !lockActive || !bubbleShown || costBubbleActive) {
      if (lockTickTimer) { clearInterval(lockTickTimer); lockTickTimer = null }
      return
    }
    var t = peakRemainText()
    if (t === null) { lockView = 'lock'; applyLockView(); scheduleLockCarousel(); return }
    if (amountEl.textContent !== t) amountEl.textContent = t
  }, 1000)
}
function scheduleLockCarousel() {
  if (lockCarTimer) { clearTimeout(lockCarTimer); lockCarTimer = null }
  if (!lockActive || !bubbleShown || lockCarouselMs <= 0 || costBubbleActive) return
  lockCarTimer = setTimeout(function () {
    lockCarTimer = null
    if (!lockActive || !bubbleShown || costBubbleActive) return
    lockView = lockView === 'lock' ? 'countdown' : 'lock'
    applyLockView()
    if (lockView === 'countdown') startCountdownTick()
...
// 距当前高峰时段结束（12:00 / 18:00，北京时间）的剩余时间，格式 HH:MM:SS；非峰时返回 null
function peakRemainText() {
  ...
  var endMs = Date.UTC(bj.getUTCFullYear(), bj.getUTCMonth(), bj.getUTCDate(), endHour, 0, 0)
  var remain = Math.max(0, endMs - bj.getTime())
...
// 常驻模式（单选）：'peak' | 'balance' | 'chat' | null
// 兼容旧字段迁移：优先读 persistMode，其次读旧布尔字段
var persistModeValue = null
function persistMode() {
  return persistModeValue
}
...
// chat 模式轮播计时：每 BUBBLE_MS 换一组台词
var chatTimer = null
function scheduleChat() {
  if (chatTimer) { clearTimeout(chatTimer); chatTimer = null }
  if (persistMode() !== 'chat' || !bubbleShown || !bubbleOn) return
  chatTimer = setTimeout(function () {
    chatTimer = null
    if (persistMode() !== 'chat' || !bubbleShown || !bubbleOn) return
    if (costBubbleActive || lockActive) return
    showChatLines()
    scheduleChat()
  }, BUBBLE_MS)
}
function saveConfig() {
  try {
    fetch(SIZE_URL, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scale: state.scale, sound: soundOn, vol: soundVol, soundSet: soundSet, usageMode: usageMode, peakMode: peakMode, bubbleOn: bubbleOn, persistMode: persistMode(), peakLockOn: peakLockOn, turnCostOn: turnCostOn, turnCostCloseMs: turnCostCloseMs, lockCarouselMs: lockCarouselMs, scrollGapOn: scrollGapOn, scrollGapPx: scrollGapPx }) })
...
// 常驻模式单选（下拉框）：null/'peak'/'balance'/'chat'
function setPersistMode(mode) {
  if (mode === 'peak' || mode === 'balance' || mode === 'chat') persistModeValue = mode
  else persistModeValue = null
  persistSelect.value = persistModeValue !== null ? persistModeValue : ''
  saveConfig()
  if (bubbleOn && persistModeValue !== null) showBase()
  else if (bubbleShown) {
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    // 常驻模式：优先读 persistMode（下拉框）；兼容旧格式 bubbleStayOn（balance）/peakShowOn（peak）
    {
      var pm = null
      if (d && typeof d.persistMode === 'string' && (d.persistMode === 'peak' || d.persistMode === 'balance' || d.persistMode === 'chat')) pm = d.persistMode
      else if (d && d.peakShowOn === true) pm = 'peak'
      else if (d && d.bubbleStayOn === true) pm = 'balance'
      persistModeValue = pm
      persistSelect.value = pm !== null ? pm : ''
    }
...
function apply(ctx) {
    ...
    function normalizePersistMode(parsed) {
      if (parsed && typeof parsed.persistMode === 'string') {
        if (parsed.persistMode === 'peak' || parsed.persistMode === 'balance' || parsed.persistMode === 'chat') return parsed.persistMode
        return null
      }
      if (parsed && parsed.peakShowOn === true) return 'peak'
      if (parsed && parsed.bubbleStayOn === true) return 'balance'
      return null
    }
        ...
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            return {
              ...
              lockCarouselMs: typeof parsed.lockCarouselMs === 'number' ? parsed.lockCarouselMs : 5000,
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, persistModeVal, peakLockOn, turnCostOn, turnCostCloseMs, lockCarouselMs, scrollGapOn, scrollGapPx) {
      ...
      const pmo = persistModeVal === 'peak' || persistModeVal === 'balance' || persistModeVal === 'chat' ? persistModeVal : null
      ...
      const lcm = typeof lockCarouselMs === 'number' ? (lockCarouselMs > 0 ? lockCarouselMs : 0) : 5000
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.persistMode, parsed.peakLockOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.lockCarouselMs, parsed.scrollGapOn, parsed.scrollGapPx)
