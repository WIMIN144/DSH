...
// Token 用量统计账本（按 模型 × 今日/本月 分桶），挂件「今日/本月用量」泡数据源
const TOKENS_FILE = path.join(DSH_HOME, '.dshw-tokens.json')
// 平台网页会话令牌的自动同步存储（官网页面上的油猴脚本推送到本地端点，用于官方校准）
...
// W 魔改版面板设置的**服务端镜像**：组结构 / 常驻勾选 / 轮播秒数 / 峰谷开关 / 触发泡文案 / 条目安排。
// 这些设置历史上只活在浏览器 localStorage，换浏览器或换机器就整体丢失（表现为「组管理看得见组、
// 常驻却读不到组」）。前端仍以 localStorage 为同步存储，启动时从本文件回填、写入时上推。
const W_SETTINGS_FILE_CANDIDATES = [
  path.join(DSH_HOME, '.dshw-w.json'),
  path.join(DSH_HOME, 'profiles', 'web', '.dshw-w.json'),
]
...
// 图库上传单张上限（前端 bubbleUploadImg 用同一个数字，别只改一边）
const BUBBLE_IMG_MAX_BYTES = 32 * 1024 * 1024
// 图库/角色图的实际格式 → Content-Type（jpg 是 W 版新放的，之前只认 png/gif）
function bubbleImgMime(fmt) {
  if (fmt === 'gif') return 'image/gif'
  if (fmt === 'jpg') return 'image/jpeg'
  return 'image/png'
}
// 落盘扩展名必须和 format 一一对应：只按「gif 之外都是 png」找文件，jpg 会存进去却永远读不到、
// 删除时还留孤儿文件（路由名固定是 bubble-img.png，靠索引里的 format 决定真实文件）。
function bubbleImgExt(fmt) {
  if (fmt === 'gif') return 'gif'
  if (fmt === 'jpg') return 'jpg'
  return 'png'
}
// 令牌类落盘一律 0600：POSIX 下默认 0644 会让同机其他账号直接读到平台 JWT。
// writeFileSync 的 mode 只在**创建**时生效，已存在的文件要再 chmod 一次。
function writePrivate(p, body) {
  fs.writeFileSync(p, body, { encoding: 'utf8', mode: 0o600 })
  try { fs.chmodSync(p, 0o600) } catch (err) {}
}
// 真实格式以字节为准：dataURL 的 MIME 是浏览器按文件扩展名给的，改过后缀或无后缀的文件
// 会报错的类型。入库前认一次魔数，存出去的扩展名/Content-Type 才和文件内容对得上。
...
// 会检查当年是否已覆盖并给出提醒）。
// 法定节假日全天谷价（官方 2026-09-19 说明）。日期表**只保留下面 HOLIDAY_VALLEY_DAYS 一份**
// （按年分键，年度维护见 DEV-NOTES §6）；上游 0.3.8 自带的那份平铺表已并入，避免两处各维护一份、
// 也避免它那个同名 isHolidayValley(bjDate) 把我们这个 isHolidayValley(timeSec) 覆盖掉。
...
const HOLIDAY_VALLEY_DAYS = {
  2026: [
    '2026-01-01', '2026-01-02', '2026-01-03', // 元旦
    '2026-02-15', '2026-02-16', '2026-02-17', '2026-02-18', '2026-02-19', '2026-02-20', '2026-02-21', '2026-02-22', '2026-02-23', // 春节（补班 2-14、2-28 为周末，仍走周末规则）
    '2026-04-04', '2026-04-05', '2026-04-06', // 清明
    '2026-05-01', '2026-05-02', '2026-05-03', '2026-05-04', '2026-05-05', // 劳动节
    '2026-06-19', '2026-06-20', '2026-06-21', // 端午
    '2026-09-25', '2026-09-26', '2026-09-27', // 中秋（不调休）
    '2026-10-01', '2026-10-02', '2026-10-03', '2026-10-04', '2026-10-05', '2026-10-06', '2026-10-07', // 国庆（补班 9-20、10-10 为周末，仍走周末规则）
  ],
}
...
function isHolidayValley(timeSec) {
  // 官方这条「法定节假日全天谷价」是 2026-09-19 才生效的规则：早于它的历史分桶按旧规则
  // （只有周末谷价）计价，否则拉早年报表时会把 1/5 月的工作日节假日错算成谷价
  if (!(Number(timeSec) >= HOLIDAY_VALLEY_FROM_SEC)) return false
  const key = beijingDayKey(timeSec)
  const list = HOLIDAY_VALLEY_DAYS[Number(key.slice(0, 4))]
  return Array.isArray(list) && list.indexOf(key) !== -1
}
// 下发给前端的节假日合并表（平铺全年份），前端峰谷倒计时据此与宿主同口径
function holidayValleyDaysFlat() {
  const out = []
  for (const y of Object.keys(HOLIDAY_VALLEY_DAYS)) {
    const list = HOLIDAY_VALLEY_DAYS[y]
    if (Array.isArray(list)) out.push(...list)
  }
  return out
}
// 下发给前端（挂件）的节假日清单：由上面那张年分键表摊平，单一来源
const HOLIDAY_VALLEY_LIST = holidayValleyDaysFlat().sort()
// 表是否已过期:今天(北京日历日)晚于表内最后一天 → 之后的法定节假日不再被覆盖,提醒维护
function holidayTableStale(timeSec = Math.floor(Date.now() / 1000)) {
  const key = beijingDayKey(timeSec)
  const flat = holidayValleyDaysFlat()
  if (!flat.length) return true
  return flat.every((d) => d < key)
}
function isPeakTime(timeSec) {
  ...
  if (isHolidayValley(n)) return false // 法定节假日全天空闲（含与周末规则叠加的冗余防御）
  // 法定节假日全天谷价（含调休放假的工作日）
...
// 注意：这里**不带** Access-Control-Allow-Origin。挂件与 dsh 页面同源，通配头只会把
// 余额、账本、模型表降级成"任意网页可读"。唯一的跨源消费者是平台令牌同步端点，
// 它按校验通过的 Origin 单独回显（见 TOKEN_SYNC_* 头）。
const JSON_HEADERS = {
  'Content-Type': 'application/json; charset=utf-8',
  'Cache-Control': 'no-store',
}

// 平台令牌同步端点的 origin 白名单（见 /dsh-whale/platform-token）。
// Origin 优先；Origin 缺席时退回 Referer（GM_xmlhttpRequest 会带页面 URL）。两者都没有即拒绝。
const PLATFORM_TOKEN_ORIGIN = 'https://platform.deepseek.com'
let platformTokenAnonWarned = false // 无来源头的放行只提示一次，避免刷屏
function platformTokenOrigin(req) {
  const h = req && req.headers
  const cand = [h && h.origin, h && h.referer]
  for (let i = 0; i < cand.length; i++) {
    const raw = cand[i]
    if (!raw) continue
    try {
      const o = new URL(String(raw)).origin
      return o === PLATFORM_TOKEN_ORIGIN ? PLATFORM_TOKEN_ORIGIN : ''
    } catch (err) {
      return ''
    }
  }
  ...
  apply(ctx) {
    ...
    function finalizeTurn(sessionId) {
      ...
      if (agg && agg.cost > 0) {
        ...
        // models 随 lastTurn 一起给前端:挂件据此判定「本轮用的是不是 DeepSeek」
        // (今日/本月用量泡是 DeepSeek 特化的,可在组管理里设为非 DeepSeek 模型时不显示)
        lastTurn = { turn: agg.turn, amount: agg.cost, tokens: agg.tokens, ts: agg.lastTs, models: modelNames }
        lastTurnSeq++
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        agg.lastTs = Date.now()
        // 用量统计账本（今日/本月用量明细泡的数据源）：按模型累计 token 分项与分项金额
        // （缓存/未命中/输出各按自己的档位计价）。reasoning ⊆ output，只记账不单独计价，
        // 用于校准指示灯的全量 token 口径；只统计 DeepSeek 模型（其他模型不计入）
        if ((cache || input || output || reasoning) && /deepseek/i.test(model)) {
          const costCacheL = (cache / 1e6) * p.hit[off]
          const costMissL = (input / 1e6) * p.miss[off]
          const costOutL = (outputBilled / 1e6) * p.out[off]
          for (const period of ['today', 'month']) {
            ...
            b.costCache += costCacheL
            b.costMiss += costMissL
            b.costOut += costOutL
          }
        }
      } catch (err) {}
    ...
    // —— Token 用量账本（W.2 迁移）：按 模型 × 今日/本月 分桶 ——
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          const sanitize = (periodMap) => {
            for (const k of Object.keys(periodMap || {})) {
              const b = periodMap[k] || {}
              for (const f of ['cache', 'miss', 'out', 'reasoning', 'costCache', 'costMiss', 'costOut']) {
                b[f] = Number(b[f]) || 0
              }
              periodMap[k] = b
            }
    ...
    function tokenBucket(period, model) {
      ...
      return p[model] || (p[model] = { cache: 0, miss: 0, out: 0, reasoning: 0, costCache: 0, costMiss: 0, costOut: 0 })
    }
    function deepseekOnly(map) {
      const out = {}
      for (const k of Object.keys(map || {})) {
        if (/deepseek/i.test(k)) out[k] = map[k]
      }
      return out
    }

    // —— 官方校准（W.2 迁移）：平台令牌 + 官方用量接口 滚动校准本地账本 ——
    // 令牌可能是多层 JSON 包装（页面 localStorage 的 userToken 本体是 {"value":...}），逐层拆开
    ...
    function normalizeOfficialSeries(data) {
      ...
      // 官方会把账号下全部模型都下发，没调用过的模型用量全 0，不进展示
    ...
    // 官方拉取失败：原因写入 lastOfficialError（接口 officialError 字段）+ 打到终端日志；
    // 同一条原因 10 分钟内不重复打印（页面每 60s 轮询一次,不节流会刷屏）
    function failOfficial(msg) {
      const now = Date.now()
      if (lastOfficialError !== msg || now - lastOfficialLogAt > 10 * 60 * 1000) {
        lastOfficialError = msg
        lastOfficialLogAt = now
        try { console.log('[dsh-whale] 官方用量拉取失败：' + msg) } catch (err) {}
      }
      return null
    }
    let officialCache = null
    let lastOfficialLogAt = 0 // 同一条失败原因的最近一次落日志时刻(10 分钟节流)
    let lastOfficialFailAt = {} // 时段键 → 上次真实请求失败时刻(2 分钟失败退避)
    // 服务端直调官网控制台后台接口：Bearer 优先、401 回退裸令牌再试一次；15s 超时；错误带上游 msg
    async function fetchOfficialRange(startSec, endSec) {
      // 缓存键的 end 必须对齐到请求实际使用的本地零点:endSec 是秒级实时值,直接拼进键会让缓存永远失配,
      // 每次页面轮询(60s)都在真打官方接口
      const endAligned = startSec + Math.ceil(Math.max(1, endSec - startSec) / 86400) * 86400
      const key = startSec + '-' + endAligned
      ...
      // 失败退避:同一时段 2 分钟内不重复真请求,避免官方接口持续失败时刷屏+刷请求
      if (lastOfficialFailAt[key] && Date.now() - lastOfficialFailAt[key] < 120000) return null
      let pt
      try {
      ...
      let lastStatus = 0
      lastOfficialFailAt[key] = Date.now() // 记录本次真实请求时刻(成败由后续分支覆盖)
      for (const scheme of schemes) {
        ...
        if (!models) {
          // 平台业务层错误是 HTTP 200 + code!=200（如 40003 令牌无效）；code 0 = 请求成功但该时段无出账数据
          ...
          return failOfficial('官方响应无用量数据（该时段可能还没出账）')
        }
        ...
        delete lastOfficialFailAt[key] // 成功后解除该时段的失败退避
    ...
    // 校准阈值（可自定义,W 面板「校准」输入框,存 size.json calibRatio,90-100 整数默认 90）:
    // 官方用量达到本地账本的该百分比才提交校准基线——官方明显滞后说明平台还没算完,沿用旧基线避免把用量校丢
    function calibRatioPct() {
      const c = readSizeConfig()
      return normalizeCalibRatio(c ? c.calibRatio : 90)
    }
    ...
    function calibrateOk(officialMap, localMap) {
      ...
      return bucketsTokenTotal(officialMap) >= local * (calibRatioPct() / 100)
    }
    async function refreshOfficialCorrection() {
      ...
      if (!(rd && rd.fresh) && !(rm && rm.fresh)) return officialCorrection // 官方数据未更新：基线不动
      const od = rd ? rd.models : null
      const om = rm ? rm.models : null
      // 今日/本月各自独立校准：今日官方常有分钟级滞后，不能连累本月一起放弃；
    ...
    // 本地账本兜底：官方出账有延迟时，「官方锚 + 本地增量」可能低于本地实时账本，
    // 按 模型×周期 比较 token 总数，本地更高时整桶换成本地账本（官方金额如有仍带上）
    ...
    function usageRecordsPayload() {
      ...
      return {
        ok: true, version: '0.3.5-w.3', today: todayData, days7,
    ...
    function writeApiRegistry(reg) {
      ...
      for (const p of API_FILE_CANDIDATES) {
        try { writePrivate(p, body); return true } catch (err) {}
      }
      return false
    }
    ...
    async function apiFetchJson(url, auth, key) {
      ...
      assertSafeApiUrl(u)
      const res = await fetch(u, { headers, redirect: 'error', signal: AbortSignal.timeout(15000) }).catch((err) => {
        // 这是唯一带用户密钥外发的请求：assertSafeApiUrl 只校验了首跳地址，跟不跟重定向由
        // fetch 决定，一个公网域名 302 到 169.254.169.254 就能把凭据送出本机。
        const msg = String((err && err.message) || err)
        if (/redirect/i.test(msg)) throw new Error('接口地址返回了重定向，挂件不跟随（防止密钥被带到别的域名）；请把 Base URL 填成跳转后的最终地址')
        throw err
      })
    ...
    function publicBalance(payload) {
      ...
      const nowSec = Math.floor(Date.now() / 1000)
      return {
        ...visible, version: '0.3.5-w.3', isPeak: isPeakTime(nowSec),
        ...
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            return {
              ...
              calibRatio: normalizeCalibRatio(parsed.calibRatio),
            }
          }
        } catch (err) {}
      }
    ...
    // 校准阈值:90-100 的整数,默认 90(官方用量达到本地账本的该百分比才提交校准基线)
    function normalizeCalibRatio(v) {
      const n = Math.round(Number(v))
      return n >= 90 && n <= 100 ? n : 90
    }

    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, menuBtnHide, codexStatsOnArg, calibRatio) {
      ...
      const cr = normalizeCalibRatio(calibRatio)
    ...
    // 当前选择的模型：输入框下方的模型选择器把它写进 $DSH_HOME/settings.yaml 的 agent-default-model。
    // 挂件用它判断「用量泡该不该显示」——今日/本月用量是按 DeepSeek 计价的特化泡，
    // 切到别的模型时就该立刻藏起来，而不是等一轮对话结束才有反应。
    let activeModelCache = { mtime: 0, value: '' }
    function readActiveModel () {
      const f = path.join(DSH_HOME, 'settings.yaml')
      try {
        const st = fs.statSync(f)
        if (activeModelCache.value && st.mtimeMs === activeModelCache.mtime) return activeModelCache.value
        const txt = fs.readFileSync(f, 'utf8')
        let model = ''
        const blk = /(?:^|\n)agent-default-model:[ \t]*\r?\n((?:[ \t]+\S[^\n]*\r?\n?)+)/.exec(txt)
        if (blk) {
          const mv = /\bmodel:[ \t]*([^\s,}\n]+)/.exec(blk[1])
          if (mv) model = mv[1].replace(/^["']|["']$/g, '')
        }
        if (!model) {
          const flow = /agent-default-model:[ \t]*\{([^}]*)\}/.exec(txt)
          if (flow) {
            const mv2 = /\bmodel:[ \t]*([^,\s}]+)/.exec(flow[1])
            if (mv2) model = mv2[1].replace(/^["']|["']$/g, '')
          }
        }
        activeModelCache = { mtime: st.mtimeMs, value: model }
        return model
      } catch (err) { return '' }
    }
    ...
    disposers.push(registerRoute({
      ...
      handler: (req, res) => {
        ...
        const payload = lastTurn
          ? { ok: true, seq: lastTurnSeq, turn: lastTurn.turn, amount: lastTurn.amount, tokens: lastTurn.tokens, ts: lastTurn.ts, models: lastTurn.models || [], activeModel: readActiveModel() }
          : { ok: true, seq: 0, turn: null, amount: null, tokens: null, ts: null, models: [], activeModel: readActiveModel() }
        res.writeHead(200, JSON_HEADERS)
        res.end(JSON.stringify(payload))
      },
    }))
    ...
    disposers.push(registerRoute({
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(
              ...
              pickN(parsed.calibRatio, old.calibRatio, 90)
            )
            // 配置刚变（可能刚开/关 Codex 统计）→ 丢掉旧快照，下一次请求立刻反映新设置
        ...
        res.writeHead(200, JSON_HEADERS)
        // 顺带下发法定节假日表:前端峰谷倒计时与宿主计价同口径(表在宿主维护,每年更新一处);
        // holidayTableStale=true 表示表已过期,前端据此弹年度维护提醒
        res.end(JSON.stringify(Object.assign({}, readSizeConfig() || {}, { holidays: holidayValleyDaysFlat(), holidayTableStale: holidayTableStale() })))
      },
    }))

    // —— 今日/本月用量明细数据源（W.2 迁移）：本地账本 + 官方校准合并 ——
    disposers.push(registerRoute({
      kind: 'exact',
      ...
      handler: async (req, res) => {
        ...
        // 官方校准：本地月桶没有历史（装了账本才开始记），有官方锚时今日/本月都以官方为准，
        // 快照之后的本地新增原样叠加；拉不到官方时如实回退纯本地
        ...
        // calib：校准指示灯状态——gray=未配置平台令牌；yellow=已配置但有待校准的新增量；green=已按官方校准
        let calib
        ...
        res.end(JSON.stringify({
          ...
          // 校准拉取官方失败回退本地时，附带失败原因供排查
          officialError: pt && source === 'local' ? (lastOfficialError || '官方数据不可用') : undefined,
        }))
      },
    }))

    // 平台令牌自动同步：官网页面上的油猴脚本把最新 userToken POST 到这里（静默期免手动抓令牌）。
    // 注意：不走 registerRoute 的信任栅栏——请求来自 platform.deepseek.com（外站油猴脚本），
    // 本地 dsh 页面的认证栅栏会拦掉它；这里自己校验 Origin/Referer 必须是平台官网。
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/platform-token',
      handler: async (req, res) => {
        // 只接受 platform.deepseek.com 的**精确** origin：
        //   · 前缀 indexOf 会被 https://platform.deepseek.com.evil.tld 绕过；
        //   · 缺 Origin 且缺 Referer 一律拒绝（浏览器发起的跨源 POST 必带其一，
        //     GM_xmlhttpRequest 会把页面 URL 填进 Referer），否则任意本地页面都能投毒令牌。
        const allowedOrigin = platformTokenOrigin(req)
        const sentOrigin = String((req.headers && req.headers.origin) || '')
        const sentReferer = String((req.headers && req.headers.referer) || '')
        if (!allowedOrigin) {
          // 带了来源头却不精确匹配（含 platform.deepseek.com.evil.tld、chrome-extension://、
          // 以及无 Origin 的跨站表单 + 外站 Referer）→ 一律拒绝，不再退回下一项。
          if (sentOrigin || sentReferer) {
            res.writeHead(403, JSON_HEADERS)
            res.end(JSON.stringify({ ok: false, error: 'forbidden origin' }))
            return
          }
          // 两个头都没有：只有"脚本管理器后台直发"这一种正常来源会走到这里（GM_xmlhttpRequest
          // 在部分浏览器不带页面 Origin）。放行以保住令牌同步，但**不回任何 CORS 头**
          // ——调用方读不到响应体，同时打一条日志，让"本机被写入"在 dsh 控制台留痕。
          if (!platformTokenAnonWarned) {
            platformTokenAnonWarned = true
            try { console.warn('[whale-balance] 平台令牌同步：收到无 Origin/Referer 的写入请求（正常应为油猴脚本），已放行并记录一次') } catch (err) {}
          }
        }
        const corsHeaders = allowedOrigin
          ? Object.assign({}, JSON_HEADERS, { 'Access-Control-Allow-Origin': allowedOrigin, Vary: 'Origin' })
          : JSON_HEADERS
        ...
        // 只有 POST 能写：漏了这一步，任何不带来源头的本机请求（GET/PUT，或表单预检之外的
        // 奇怪方法）都会走到下面的写入分支。
        if (req.method !== 'POST') {
          res.writeHead(405, Object.assign({}, corsHeaders, { Allow: 'POST, OPTIONS' }))
        ...
        try {
          ...
          writePrivate(PLATFORM_TOKEN_FILE, JSON.stringify({ token: token, updatedAt: new Date().toISOString() }))
    ...
    // 端口按请求 Host 动态注入（__PORT__ 占位），换端口无需改脚本
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/token-sync.user.js',
      handler: (req, res) => {
        try {
          let src = fs.readFileSync(path.join(PACKAGE_ROOT, 'assets', 'dsh-whale-token-sync.user.js'), 'utf8')
          let port = ''
          try {
            const h = String(req.headers.host || '')
            const m = /:(\d+)$/.exec(h)
            port = m ? m[1] : (h && /^\d+$/.test(h) ? h : '')
          } catch (err) {}
          src = src.replace(/__PORT__/g, port || '8080')
        ...
        try {
          if (req.method === 'PUT' || req.method === 'POST') {
            ...
            if (action === 'set-key' || action === 'delete-key') {
              ...
              // 凭据名要写进 .credentials.yaml 的 refs 段：放开字符集就等于允许构造
              // `foo: x`、换行、`..` 之类的键去改别人的条目。命名约定本来就是大写蛇形。
              if (!/^[A-Za-z0-9_.\-]{1,64}$/.test(ref)) throw new Error('bad keyRef（只允许字母、数字、_ . -，长度 1~64）')
      ...
      handler: async (req, res) => {
        try {
          if (req.method === 'POST' || req.method === 'PUT') {
            ...
            // W.3：free = 自定义泡泡自由列表（不占点击顺序，供组管理分组；元素须为 {kind, modules} 形状的对象）
            const free = Array.isArray(parsed.free) ? parsed.free.filter(function (f) { return f && typeof f === 'object' && Array.isArray(f.modules) }) : []
            const cfg = { v: 1, items: parsed.items, lib: parsed.lib, tapAdvance: parsed.tapAdvance === true, free: free }
    ...
    // —— W 面板设置镜像（见 W_SETTINGS_FILE_CANDIDATES 注释）——
    // 只放行白名单键并逐个校验形状；不合法的键丢弃并在响应里回报，避免脏数据永久占住镜像
    const W_SETTINGS_VALID = {
      dshwWGroupsV5: (v) => Array.isArray(v) && v.length > 0 && v.length <= 64 && v.every((g) => g && typeof g.id === 'string' && typeof g.name === 'string' && Array.isArray(g.items) && g.items.length <= 512 && g.items.every((x) => typeof x === 'string')),
      dshwWPersistGroups: (v) => !!v && typeof v === 'object' && !Array.isArray(v),
      dshwWCarousel: (v) => typeof v === 'number' && isFinite(v) && v >= 0 && v <= 99,
      dshwWTriggers: (v) => !!v && typeof v === 'object' && !Array.isArray(v),
      dshwWTrigMods: (v) => !!v && typeof v === 'object' && !Array.isArray(v),
      dshwWTrigAssign: (v) => !!v && typeof v === 'object' && !Array.isArray(v),
      dshwWEntryOff: (v) => !!v && typeof v === 'object' && !Array.isArray(v),
      dshwWEntryW: (v) => !!v && typeof v === 'object' && !Array.isArray(v) && Object.keys(v).length <= 512 && Object.keys(v).every((k) => typeof v[k] === 'number' && v[k] >= 1 && v[k] <= 99),
      dshwWCarouselMode: (v) => v === 'random' || v === 'order',
      dshwWEco: (v) => v === 'deepseek' || v === 'zhipu' || v === 'qwen',
      dshwWEcoLock: (v) => v === 0 || v === 1, // localStorage 里的 '0'/'1' 会被 JSON.parse 成数字
      dshwWEntryName: (v) => {
        if (!v || typeof v !== 'object' || Array.isArray(v)) return false
        const ks = Object.keys(v)
        return ks.length <= 512 && ks.every((k) => typeof v[k] === 'string' && v[k].length <= 40)
      },
      dshwWEntryTag: (v) => {
        // {id: ['deepseek','qwen']} —— 多选生态标，空数组 = 「无」
        if (!v || typeof v !== 'object' || Array.isArray(v)) return false
        const ks = Object.keys(v)
        return ks.length <= 512 && ks.every((k) => Array.isArray(v[k]) && v[k].length <= 3 &&
          v[k].every((t) => ['deepseek', 'zhipu', 'qwen'].indexOf(t) !== -1))
      },
    }
    function loadWSettings() {
      for (const p of W_SETTINGS_FILE_CANDIDATES) {
        try {
          const parsed = JSON.parse(fs.readFileSync(p, 'utf8'))
    ...
    function writeWSettings(settings) {
      const body = JSON.stringify({ v: 1, updatedAt: new Date().toISOString(), settings }, null, 2)
      for (const p of W_SETTINGS_FILE_CANDIDATES) {
        try { fs.writeFileSync(p, body, 'utf8'); return true } catch (err) {}
      }
      return false
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/w-settings.json',
      handler: async (req, res) => {
        try {
          if (req.method === 'POST' || req.method === 'PUT') {
            const parsed = JSON.parse(await readBodyMax(req, 256 * 1024))
            const incoming = parsed && typeof parsed === 'object' ? parsed.settings : null
            if (!incoming || typeof incoming !== 'object' || Array.isArray(incoming)) {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: 'invalid w settings' }))
              return
            }
            // 合并写：只覆盖本次带上来的键。整机 localStorage 里没动过的设置（例如从没
            // 换过轮播方式）不会出现在提交体里，整包替换会把另一台机器存好的那份抹掉。
            const stored = loadWSettings() || {}
            const next = {}
            const badKeys = []
            for (const key of Object.keys(W_SETTINGS_VALID)) {
              if (!(key in incoming)) {
                if (key in stored) next[key] = stored[key]
                continue
              }
              if (W_SETTINGS_VALID[key](incoming[key])) next[key] = incoming[key]
              else {
                badKeys.push(key)
                if (key in stored) next[key] = stored[key] // 形状不合法就保留上一份好的，别把镜像清掉
              }
            }
            if (!writeWSettings(next)) {
              res.writeHead(500, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: 'w settings write failed' }))
              return
            }
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify({ ok: true, settings: next, rejected: badKeys }))
            return
          }
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify({ ok: true, settings: loadWSettings() }))
        } catch (err) {
          res.writeHead(400, JSON_HEADERS)
          res.end(JSON.stringify({ ok: false, error: String((err && err.message) || err).slice(0, 200) }))
        }
    ...
    disposers.push(registerRoute({
      ...
      handler: async (req, res) => {
        try {
          const body = await readBodyMax(req, 48 * 1024 * 1024) // base64 会膨胀 4/3，32MB 原图 ≈ 43MB 请求体
          const parsed = JSON.parse(body)
          ...
          if (action === 'upload') {
            ...
            const m = /^data:image\/(png|gif|jpe?g);base64,([A-Za-z0-9+/=]+)$/i.exec(data)
            if (!m) {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: '只支持 png / gif / jpg' }))
              return
            }
            let format = m[1].toLowerCase() === 'gif' ? 'gif' : (/jp/.test(m[1].toLowerCase()) ? 'jpg' : 'png')
            const buf = Buffer.from(m[2], 'base64')
            if (buf.length < 64) {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: '图片内容太小，像是空文件' }))
              return
            }
            const sniffed = sniffImageFormat(buf)
            if (!sniffed) {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: '文件内容和 png / gif / jpg 对不上，可能被改过后缀' }))
              return
            }
            format = sniffed
            if (buf.length > BUBBLE_IMG_MAX_BYTES) {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: '图片 ' + Math.round(buf.length / 1048576) + 'MB，超过 ' + Math.round(BUBBLE_IMG_MAX_BYTES / 1048576) + 'MB 上限' }))
              return
            }
            const id = 'bimg_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8)
            fs.writeFileSync(path.join(pickBubbleImgDir(), id + '.' + bubbleImgExt(format)), buf)
          ...
          if (action === 'delete') {
            ...
            writeBubbleImgIndex(index)
            try { fs.unlinkSync(path.join(pickBubbleImgDir(), id + '.' + bubbleImgExt(img.format))) } catch (err) {}
            res.writeHead(200, JSON_HEADERS)
            res.end(JSON.stringify(bubbleImgPayload()))
            return
          }
    ...
    disposers.push(registerRoute({
      ...
      handler: (req, res) => {
        try {
          ...
          const img = index.images.find((x) => x.id === id)
          if (img) {
            const bytes = fs.readFileSync(path.join(pickBubbleImgDir(), id + '.' + bubbleImgExt(img.format)))
            res.writeHead(200, {
              'Content-Type': bubbleImgMime(img.format),
              'Cache-Control': 'no-store',
              'Content-Length': String(bytes.length),
            })
            res.end(bytes)
          ...
          if (def) {
            ...
            if (bytes) {
              res.writeHead(200, {
                'Content-Type': bubbleImgMime(def.format),
                'Cache-Control': 'no-store',
                'Content-Length': String(bytes.length),
              })
              res.end(bytes)
