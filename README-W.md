...
- 标题旁指示灯：**灰** = 本机没有可用的平台令牌；**黄** = 已有令牌、但还有没校准进来的新增；**绿** = 已按官方数据校准。⚠️ 灯看的是**本机存的令牌**（`~/.dsh/.dshw-platform-token.json`），不是"浏览器里有没有装脚本" —— 把油猴脚本删掉，灯在令牌过期前仍然是绿的；反过来，重装插件、换浏览器都不会让它变灰或变绿。
...
**灰灯时数字为什么比官网小一截**：那时显示的是**纯本地账本**，只包含这台机器上被记进账本的调用 —— 装挂件之前的历史、换过设备、换过对话的部分都不在内，所以通常会明显小于 platform 后台的数。另外「本月」桶按自然月累加，**不会因为当天没调用就清零**：今天没跑模型，今日泡显示「暂无统计」，而本月泡仍是本月累计值，这是正常的。要和官网对上，就配下面的令牌。
...
- **指示灯是灰的？** 本机没有可用的平台令牌，用量为纯本地账本口径；按第四节三步配置即可。
- **删了油猴脚本，灯怎么还是绿的？** 正常。灯看的是本机 `~/.dsh/.dshw-platform-token.json` 里的令牌，删脚本只是"以后不再自动更新令牌"，已存的令牌在过期前照样能用。想让它变灰：删掉那个文件（或等它过期）。
- **本月用量比官网小很多？** 灰灯状态下就是本地账本口径，只含这台机器记到的调用，装挂件之前和别的设备上的历史都不在内，偏小是正常的；配好令牌、完成一次校准后会对齐官网。
