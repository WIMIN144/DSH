...
> **本目录是 W 魔改版 `dsh-whale-widget-w`**（原版 `dsh-whale-widget` 的功能增强分支）。本文讲安装、功能与隐私口径；**W 版新增的面板、分组泡泡管理、生态打标与联锁等用法以 [README-W.md](README-W.md) 为准**，维护者视角的实现约束与坑在 [DEV-NOTES.md](DEV-NOTES.md)。
...
- 🖼️ **泡泡图库**：内置 `petpet`、`money1` 两张图，也可上传 png/gif/jpg（单张 ≤32MB；静态 png/jpg 上传前自动压缩到最长边 800px，GIF/APNG 原样保留），供图片/随机图片模块使用
...
├── README.md                 # 本文件
├── README-W.md               # W 魔改版功能说明（W 面板 / 泡泡管理 / 生态 / 用量校准）
...
│   ├── dsh-whale-token-sync.user.js
│   │                         # ★ 油猴（Tampermonkey）脚本本体 —— 平台令牌同步就是这一个文件；
│   │                         #   浏览器不是直接拿它，而是访问 http://127.0.0.1:3080/dsh-whale/token-sync.user.js
│   │                         #   由宿主下发（路由见 lib/index.js 的 token-sync.user.js）
...
├── docs/                     # 界面截图（README-W 内引用）
└── DEV-NOTES.md              # 开发者备忘录：存储键、已知坑、路由清单、年度维护
```

运行时数据（都放在 `$DSH_HOME`，默认 `~/.dsh`）：
...
- 完整规格、视觉参数、路由清单、架构结论与年度维护步骤见 [DEV-NOTES.md](DEV-NOTES.md)（开发者备忘录）。
